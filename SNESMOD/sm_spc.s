...
; some modifications by doug fraker 2022
;========================================================
...
;--------------------------------------------------------
	...
	mov	sfx_mask, #0
	mov sfx_channel, #7 ;channel
	mov sfx_pan, #7 ;pan center
	ret
...
;	.word	cmd_res			; 07h - reset
	.word	cmd_clone		; 07h - copy from song table to the sfx table
	.word	cmd_fx			; 08h - sound effect
	.word	cmd_fx2		; 09h - more sound effect params
	.word	cmd_ssize		; 0ah - set stream size
; streaming is handled with __brk_routine__
; a negative value is checked every time we run sproc (tcall 0)
...
cmd_clone:
;********************************************************
	mov a, spc_port2 ; sample id
	asl a
	asl a ;x4
	mov x, a
	
	mov	y, next_sample
	mov a, !sampledirectory+x ;song table
	mov	!effectdirectory+y, a ;sfx table
	mov a, !sampledirectory+1+x
	mov	!effectdirectory+1+y, a
	mov a, !sampledirectory+2+x
	mov	!effectdirectory+2+y, a
	mov a, !sampledirectory+3+x
	mov	!effectdirectory+3+y, a
	 
	clrc
	adc	next_sample, #4

;we don't need to change xfer_address
...
fx_pitch: 
.word $200, $211, $23F, $261, $285, $2AB, $2D4, $2FF, $32C, $35D, $390, $3C7
.word $400, $43D, $47D, $4C2, $50A, $557, $5A8, $5FE, $659, $6BA, $721, $78D
.word $800, $87a, $8fb, $983, $A14, $AAE, $B50, $BFD, $CB3, $D74, $E41, $F1A
.word $1000, $10F4, $11F6, $1307, $1429, $155C, $16A1, $17F9, $1966, $1AE9, $1C82, $1E34
.word $2000, $21E7, $23EB, $260E, $2851, $2AB7, $2D3E, $2FF2, $32CC, $35D1, $3904, $3C68
.word $3FFF	 ;values 0-60
...
;constant power pan
;sine() at 6 degree shifts, with 127 max output
pan_table:
.byte 0, 13, 26, 39
.byte 52, 64, 75, 85
.byte 94, 103, 110, 116
.byte 121, 124, 126, 127
...
; "vvvvssss" vol / sfx id
; "--pppppp" pitch
; if pitch >= $80, don't retrigger
;*************************************************************************
sfx_play:
;-------------------------------------------------------------------------
	mov m4, #0; retrigger by default
	
	mov y, sfx_pan	; m2 = pan (vol L, vol R)
	mov	a, !pan_table+y
	mov	m2+1, a			;right
	mov a, #15
	setc
...
;if m1 (gain) = 0, reset the sfx flag, koff	
	mov y, sfx_channel
	mov a, m1 ;gain
	bne _sfx_use1_ok
...
;note off	
	...
	mov a, !mask_or+y ;note off
;	mov spc_dspd, #%10000000 ;note off
	mov spc_dspd, a
	nop
	mov spc_dspd, #0 ;buggy, this seems to fix it
	
	mov a, !mask_and+y
	and a, sfx_mask ;#%01111111
	mov sfx_mask, a
	bra	_sfx_start		;
...
_sfx_use1_ok:
	...
	cmp m4, #1 
	beq _sfx_start ; no retrigger
	
	mov a, sfx_channel
	xcn a
	or a, #4
	mov	spc_dspa, a ;#074h
	mov	spc_dspd, m1+1 ;src
	mov	spc_dspa, #dsp_kon
	;y = channel
	mov a, !mask_or+y ;note on
	mov spc_dspd, a
	or a, sfx_mask
	mov sfx_mask, a
...
;-------------------------------------------------------------------------
	...
	xcn a
	and a, #0f0h
	mov	spc_dspa, a ;#070h		; setup dsp pointer
	
	mov	spc_dspd, m2		; volume l
	inc	spc_dspa		;
	mov	spc_dspd, m2+1		; volume r
	inc	spc_dspa		;
	mov	spc_dspd, m3 ;#0		; pitch l
	inc	spc_dspa		;
	mov	spc_dspd, m3+1		; pitch h
	inc	spc_dspa		;
	inc	spc_dspa		;
...
;?? testing, let the note play and loop
;	eor	a, sfx_mask		;
;	and	a, sfx_mask		;
;	mov	sfx_mask, a		;
	ret				;
